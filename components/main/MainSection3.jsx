...
const MainSection3 = () => {
  ...
  const numTimeRef = useRef();
  const numIntervalRef = useRef();
  ...
  useEffect(() => {
    if (obState) {
      numTimeRef.current = setTimeout(() => {
        numIntervalRef.current = setInterval(() => {
          setNum((state) => state + 20);
        }, 0);
      }, 1000);
    }
  }, [obState]);
  useEffect(() => {
    if (num > 2312) {
      ...
      clearTimeout(numTimeRef.current);
    }
  }, [num]);
  useEffect(() => {
    const options = {
    ...
    <MainSection3Wrap ref={obserberRef}>
      <div className="container">
        ...
        <h2 className={obState ? "active" : ""}>
          <span>
            <span>총 {num.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}회 수업</span>으로 검증된
          </span>
